...
my_vars.add_argument('-d','--odir',dest='output_dir', type=str, required=True, help='give an output directory')
...
# open input file
with open(my_files.input,'r') as input_data, open(f"{my_files.output_dir}/temporary_filtered_gammaprobacteria.txt",'w') as filtered_file:
    ...
    for line in input_data.readlines()[1:]:
        ...
        # get ride of any escape characters like (,),.,[,] in the names of the species we are using 
        genus_species_search = re.escape(genus_species)
        ...
        # otherwise if the species in the line isnt in the current species then do the re thing
            ...
            all_organisms = dict(enumerate(re.findall(rf"GC.*{genus_species_search}.*\n",full_data)))

            print(f'The genus species currently being looked at is {genus_species}, the search parameter is {genus_species_search} and all the organisms found are {all_organisms}')
            ...
            for tracker,data in all_organisms.items():
                ...
                # handle the error if coverage isnt given
                # if the coverage field isnt empty
                if data.split('\t')[3]:
                    coverage = float(data.split('\t')[3])
                
                elif not data.split('\t')[3]:
                    coverage = 0
                ...
                # check if the year is the same
                    ...
                    # check if the months are the same
                        ...
                        # if the days are the same then resort to using coverage
                            ...
                            # if there is no coverage value or the coverage is the same after all the checks to get here then the assemblies are basically the same
                            if not coverage or coverage == latest_assembly_best_covg[2]:
                                
                                # replace the current best assembly
                                latest_assembly_best_covg = [tracker,(int(year),int(month),int(day)),coverage]

                            # check if coverage is better with the current organism
                            elif coverage > latest_assembly_best_covg[2]:

                                # replace the current best assembly
                                latest_assembly_best_covg = [tracker,(int(year),int(month),int(day)),coverage]

                            # if the coverage is worse dont use what we have now
                            elif coverage < latest_assembly_best_covg[2]:
                                continue
...
# has to be here because output file has been closed now
# remove duplicates from the file. These duplicates are not created because the logic of the script is wrong but because it is actually right
os.system(f"sort {my_files.output_dir}/temporary_filtered_gammaprobacteria.txt | uniq > {my_files.output_dir}/filtered_gammaproteobacteria.txt")

# remove the unsorted file with duplicates we were using before
os.system(f'rm {my_files.output_dir}/temporary_filtered_gammaprobacteria.txt')
