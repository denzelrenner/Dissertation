...
with open(my_files.array_of_assembly_species,'r') as input_data, open(f'{my_files.output_dir}/output_data/all_species_metrics.txt','w+') as metrics_file :
    ...
    for line in input_data.readlines():
        ...
        # get the full line data except for the accession.
        species_and_stats = ' '.join(line_data[1:])

        # get the spcies name only and replace any weird characters that are not letters or numbers. So first remove the weird letters from the name then split the string based on spaces
        # this will leave some empty strings in our list of fragments
        interim_species_name = re.split(r'\s+',re.sub(r"\W+"," ",f"{line_data[1]}"))

        # remove the empty strings from the list of fragments and join everything else
        species_name = '_'.join([fragment for fragment in interim_species_name if fragment])

        # naming with underscores for file names. Join the species name and date of the assembly, and replace any weird characters
        file_name_species = species_name + '_' + line_data[2]

        # print useful info
        print(f"This species name and stats is {species_and_stats} and for files it is {file_name_species}")
